...
public class AinTimeController implements Runnable {
	...
	private void checkAinTime() {
		...
		int ainTime = Config.RATE_AIN_TIME; // 时间比例
		int ainMaxPercent = Config.RATE_MAX_CHARGE_PERCENT; // 殷海萨的祝福 百分比
		if (nowTime % ainTime == 0) {
			for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
				if (pc.getLevel() >= 49) { // 等级限制
					if (pc.getAinPoint() < ainMaxPercent
						&& pc.getMap().isSafetyZone(pc.getLocation())) { // 限制最高点数
						pc.setAinPoint(pc.getAinPoint() + 1); // 安全区域点数 +1
						pc.setAinZone(1);
