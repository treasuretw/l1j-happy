...
*/
		...
		for (int i = 0; i < shopItems.size(); i++) {
			...
			writeD(price);
			//（商店显示部分）
			if (shopItem.getPackCount() > 1 && shopItem.getEnchantLevel() > 1){
				writeS("+" + shopItem.getEnchantLevel() + " " +item.getName() + " (" + shopItem.getPackCount() + ")");
			} else {
				if (item.getItemId() == 40309) {// 食人妖精RaceTicket
					...
					if (shopItem.getEnchantLevel() > 1 && shopItem.getPackCount() < 2) {
						writeS("+" + shopItem.getEnchantLevel() + " " + item.getName());
					} else
