	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (param.equalsIgnoreCase("AC")) {
				...
				target.addAc((short) (value - target.getAc()));//突破防御限制
			}
				...
				if (param.equalsIgnoreCase("HP")) {
					...
					target.sendPackets(new S_SystemMessage("GM变更你的权限。"));
				}
					...
					pc.sendPackets(new S_SystemMessage("角色素质  " + param + " 未知。"));
					return;
				}
				target.save(); // DBにキャラクター情報を書き込む
			}
			target.sendPackets(new S_OwnCharStatus(target));
			pc.sendPackets(new S_SystemMessage(target.getName() + " 的" + param + "数值" + value + "已变更。"));
		}
		catch (Exception e) {
