...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public L1PcInstance loadCharacter(String charName) {
		...
		try {
			...
			pc.setFameLevel(rs.getInt("FamenameLevel"));// sosodemon add 称望系统 BY.SosoDEmoN
			pc.setLevelmet(rs.getInt("Levelnamemet"));// 记录转生次数
			...
			pc.setElfAttr2(rs.getInt("ElfAttr2"));// TODO 双属性
			...
			pc.setAinZone(rs.getInt("AinZone"));			// 殷海薩的祝福
			pc.setAinPoint(rs.getInt("AinPoint"));			// 殷海薩的祝福
	...
	public void createCharacter(L1PcInstance pc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO characters SET account_name=?,objid=?,char_name=?,Levelnamemet=?,FamenameLevel=?,level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,ElfAttr2=?,PKcount=?,PkCountForElf=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,Pay=?,HellTime=?,Banned=?,Karma=?,LastPk=?,LastPkForElf=?,DeleteTime=?");
			pstm.setString(++i, pc.getAccountName());
			pstm.setInt(++i, pc.getId());
			pstm.setString(++i, pc.getName());
			pstm.setInt(++i, pc.getLevelmet());// 记录转生次数
			pstm.setInt(++i, pc.getFameLevel()); // sosodemon add 称望系统 BY.SosoDEmoN
			...
			pstm.setInt(++i, pc.getElfAttr2());// TODO 双属性
	...
	public void storeCharacter(L1PcInstance pc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE characters SET Levelnamemet=?,FamenameLevel=?,level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,ElfAttr2=?,PKcount=?,PkCountForElf=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,HellTime=?,Banned=?,Karma=?,LastPk=?,LastPkForElf=?,DeleteTime=?,LastActive=?,AinZone=?,AinPoint=? WHERE objid=?");
			pstm.setInt(++i, pc.getLevelmet());// 记录转生次数
			pstm.setInt(++i, pc.getFameLevel()); // sosodemon add 称望系统 BY.SosoDEmoN
			pstm.setInt(++i, pc.getLevel());
			pstm.setInt(++i, pc.getHighLevel());
