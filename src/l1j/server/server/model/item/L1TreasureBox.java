...
public class L1TreasureBox {
	 ...
	 * @return 指定されたIDのTreasureBox。见つからなかった场合はnull。
	 */
	...
	/**
	 * TreasureBoxを开けるPCにアイテムを入手させる。PCがアイテムを持ちきれなかった场合は アイテムは地面に落ちる。
	 * 
	 * @param pc
	 *            - TreasureBoxを开けるPC
	 * @return 开封した结果何らかのアイテムが出てきた场合はtrueを返す。 持ちきれず地面に落ちた场合もtrueになる。
	 */
	...
	private static void storeItem(L1PcInstance pc, L1ItemInstance item) {
		...
		if (pc.getInventory().checkAddItem(item, item.getCount()) == L1Inventory.OK) {
			...
			// 持てない场合は地面に落とす 处理のキャンセルはしない（不正防止）
