		...
		if (talking != null) {
			if (npcid == 50014) { // ディロン
				...
				if (player.getTempCharGfx() == 1037) { // ジャイアントアント变身
					htmlid = "ants3";
				} else if (player.getTempCharGfx() == 1039) {// ジャイアントアントソルジャー变身
					if (player.isCrown()) { // 君主
						if (quest.get_step(L1Quest.QUEST_LEVEL30) == 1) {
							if (player.getInventory().checkItem(40547)) { // 住民たちの遗品
								htmlid = "antsn";
							} else {
								htmlid = "ants1";
							}
				...
				if (player.isElf()) { // エルフ
					if (quest.get_step(L1Quest.QUEST_LEVEL30) == 1) {
						if (!player.getInventory().checkItem(40592)) { // 咒われた精灵书
