...
public class L1MerchantInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			if (npcid == 70841) { // ルーディエル
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 15) {
						...
						if ((lv15_step == 2)
								|| (lv15_step == L1Quest.QUEST_END)) { // クリア济み
							htmlid = "gunterp11";
						} else {
							htmlid = "gunterp9";
						}
					...
					if (lv30_step == 0) { // 未开始
						...
						htmlid = "gunterkE1";
					} else if (lv30_step == 2) { // グンター同意济み
						htmlid = "gunterkE2";
					} else if (lv30_step >= 3) { // グンター终了济み
						htmlid = "gunterkE3";
					}
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 45) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL30)) { // lv30クリア济み
							int lv45_step = quest
									.get_step(L1Quest.QUEST_LEVEL45);
							if (lv45_step == L1Quest.QUEST_END) { // クリア济み
								htmlid = "masha4";
							} else if (lv45_step >= 1) { // 同意济み
								htmlid = "masha3";
							} else { // 未同意
								htmlid = "masha1";
							}
					...
					if (player.getLevel() >= 45) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL30)) { // Lv30クエスト终了济み
							int lv45_step = quest
									.get_step(L1Quest.QUEST_LEVEL45);
							if (lv45_step == L1Quest.QUEST_END) { // クリア济み
								htmlid = "mashak3";
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 15) {
						int lv15_step = quest.get_step(L1Quest.QUEST_LEVEL15);
						if (lv15_step == 1) { // ゼロクリア济み
							htmlid = "zero5";
						} else if (lv15_step == L1Quest.QUEST_END) { // ゼロ、グンタークリア济み
							htmlid = "zero1";// 6
						} else {
							htmlid = "zero1";
						}
				...
				if (player.isCrown()) { // 君主
					if (player.getLevel() >= 30) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL15)) { // lv15试练クリア济み
							int lv30_step = quest
									.get_step(L1Quest.QUEST_LEVEL30);
							if (lv30_step == L1Quest.QUEST_END) { // クリア济み
								htmlid = "aria3";
							} else if (lv30_step == 1) { // 同意济み
								htmlid = "aria2";
							} else { // 未同意
								htmlid = "aria1";
							}
				...
				if (player.getTempCharGfx() == 1037) {// ジャイアントアント变身
					if (player.isCrown()) { // 君主
				...
				if (player.isCrown()) { // 君主
					...
					if ((lv45_step >= 1) && (lv45_step != L1Quest.QUEST_END)) { // 开始かつ未终了
						if (player.getInventory().checkItem(40586)) { // 王家の纹章(左)
							htmlid = "richard4";
						} else {
							htmlid = "richard1";
						}
				...
				if (player.isCrown()) { // 君主
					...
					if (lv45_step == 1) {
						htmlid = "meg1";
					} else if ((lv45_step == 2) && (lv45_step <= 3)) { // メグ同意济み
						htmlid = "meg2";
					} else if (lv45_step >= 4) { // メグクリア济み
						htmlid = "meg3";
					}
				}
			} else if (npcid == 71200) { // 白魔术师 ピエタ
				if (player.isCrown()) { // 君主
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					if ((lv45_step == 2)
				...
				// //メグクリア济みor终了
				// htmlid = "pieta9";
				// } else if (lv45_step == 2) { // クエスト开始前・メグ同意济み
				// htmlid = "pieta2";
				// } else if (lv45_step == 2 ||
				// player.getInventory().checkItem(41422) ) {//
				// 辉きを失った魂保持
				// htmlid = "pieta4";
				// } else if (lv45_step == 3) { // 辉きを失った魂入后
				// htmlid = "pieta6";
				// } else {//lv45未满orクエスト30未
				// htmlid = "pieta8";
				// }
				// } else { // 君主以外
				// htmlid = "pieta1";
				...
				if (player.isKnight()) { // ナイト
					if (player.getLevel() >= 15) {
						int lv15_step = quest.get_step(L1Quest.QUEST_LEVEL15);
						if (lv15_step == L1Quest.QUEST_END) { // アノンクリア济み
							htmlid = "aanon7";
						} else if (lv15_step == 1) { // リッキークリア济み
							htmlid = "aanon4";
						}
					}
				}
				...
				if (player.isKnight()) { // ナイト
					if (player.getLevel() >= 30) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL15)) { // LV15クエスト终了济み
							int lv30_step = quest
									.get_step(L1Quest.QUEST_LEVEL30);
							if (lv30_step == 0) { // 未开始
								htmlid = "mark1";
				...
				if (player.isCrown()) { // 君主
					...
					int lv30_step = quest.get_step(L1Quest.QUEST_LEVEL30);
					if (lv30_step == L1Quest.QUEST_END) { // ゲラド终了济み
						htmlid = "gerardkEcg";
					} else if (lv30_step < 3) { // グンター未终了
						htmlid = "gerardk7";
					} else if (lv30_step == 3) { // グンター终了济み
						htmlid = "gerardkE1";
					} else if (lv30_step == 4) { // ゲラド同意济み
						htmlid = "gerardkE2";
					} else if (lv30_step == 5) { // ラミアの鳞 终了济み
						htmlid = "gerardkE3";
					} else if (lv30_step >= 6) { // 复活のポーション同意济み
						htmlid = "gerardkE4";
					}
				} else if (player.isElf()) { // エルフ
					htmlid = "gerarde1";
				...
				if (player.getTempCharGfx() == 2374) { // スケルトン变身
					if (player.isKnight()) { // ナイト
						if (quest.get_step(L1Quest.QUEST_LEVEL30) == 6) { // 复活のポーション同意济み
							htmlid = "jim2";
						} else {
							htmlid = "jim4";
						}
				...
				if (player.isKnight()) { // ナイト
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					if (lv45_step == 1) { // マシャー同意济み
						htmlid = "jimuk1";
					} else if (lv45_step >= 2) { // ジーム同意济み
						htmlid = "jimuk2";
					}
				}
				...
				if (player.isKnight()) { // ナイト
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					if (lv45_step == 2) { // ジーム同意济み
						if (player.getInventory().checkItem(20026)) { // ナイトビジョン
							htmlid = "giantk1";
						}
					} else if (lv45_step == 3) { // ジャイアントエルダー同意济み
						htmlid = "giantk2";
				...
				if (player.isElf()) { // エルフ
					if (player.getLevel() >= 30) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL15)) { // Lv15终了济み
							int lv30_step = quest
									.get_step(L1Quest.QUEST_LEVEL30);
							if (lv30_step == L1Quest.QUEST_END) { // 终了济み
								htmlid = "motherEE3";
							} else if (lv30_step >= 1) { // 同意济み
								htmlid = "motherEE2";
				...
				if (player.isElf()) { // エルフ
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					if (lv45_step >= 4) { // ヘイト终了济み
						htmlid = "heit5";
					} else if (lv45_step >= 3) { // フルート交换济み
						htmlid = "heit3";
					} else if (lv45_step >= 2) { // ヘイト同意济み
						htmlid = "heit2";
					} else if (lv45_step >= 1) { // マシャー同意济み
						htmlid = "heit1";
					}
				}
				...
				if (player.isWizard()) { // ウィザード
					if (player.getLevel() >= 15) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL15)) { // 终了济み
							htmlid = "jem6";
						} else {
							htmlid = "jem1";
						}
				...
				if (player.isCrown()) { // 君主
					...
					if (player.getLevel() >= 30) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL15)) {
							int lv30_step = quest
									.get_step(L1Quest.QUEST_LEVEL30);
							if (lv30_step >= 4) { // ゲレン终了济み
								htmlid = "gerengw3";
							} else if (lv30_step >= 3) { // 要求济み
								htmlid = "gerengT4";
							} else if (lv30_step >= 2) { // アンデッドの骨交换济み
								htmlid = "gerengT3";
							} else if (lv30_step >= 1) { // 同意济み
								htmlid = "gerengT2";
							} else { // 未同意
					...
					if (lv30_step == L1Quest.QUEST_END) {
						if (player.getLevel() >= 45) {
							int lv45_step = quest
									.get_step(L1Quest.QUEST_LEVEL45);
							if ((lv45_step >= 1)
									&& (lv45_step != L1Quest.QUEST_END)) { // 同意济み
								htmlid = "talassmq2";
				...
				if (player.isWizard()) { // ウィザード
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					if (lv45_step >= 3) { // 神秘の岩终了济み
						htmlid = "stoenm3";
					} else if (lv45_step >= 2) { // 神秘の岩 同意济み
						htmlid = "stoenm2";
					} else if (lv45_step >= 1) { // タラス 同意济み
						htmlid = "stoenm1";
					}
				}
				...
				if (player.getLevel() >= 50) {
					...
					} else if (lv50_step >= 1) { // ディガルディン 同意济み
						if (player.isCrown()) { // 君主
							htmlid = "dicardingp2";
						} else if (player.isKnight()) { // ナイト
							htmlid = "dicardingk2";
				...
				if (player.isDarkelf()) { // ダークエルフ
					if (player.getLevel() >= 15) {
						int lv15_step = quest.get_step(L1Quest.QUEST_LEVEL15);
						if (lv15_step == L1Quest.QUEST_END) { // 终了济み
							htmlid = "kanguard3";
						} else if (lv15_step >= 1) { // 同意济み
							htmlid = "kanguard2";
						} else { // 未同意
							htmlid = "kanguard1";
						}
			...
			} else if (npcid == 70895) { // 布鲁迪卡
				if (player.isDarkelf()) { // 黑暗妖精
					if (player.getLevel() >= 45) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL30)) {
							int lv45_step = quest
									.get_step(L1Quest.QUEST_LEVEL45);
							if (lv45_step == L1Quest.QUEST_END) { // 45级任务结束
								if (player.getLevel() < 50) { //  未达到 Lv50
									htmlid = "bluedikaq3";
								} else {
									int lv50_step = quest
											.get_step(L1Quest.QUEST_LEVEL50);
									if (lv50_step == L1Quest.QUEST_END) { // 50级任务结束
										htmlid = "bluedikaq8";
			...
			} else if (npcid == 70904) { // 库普
				if (player.isDarkelf()) {
					if (quest.get_step(L1Quest.QUEST_LEVEL45) == 1) { // ブルディカ同意济み
						htmlid = "koup12";
					}
				}
			} else if (npcid == 70906) { // 奇马
				if (player.isDarkelf()) { // 黑暗妖精
					if (player.getLevel() >= 50) {
						int lv50_step = quest
						.get_step(L1Quest.QUEST_LEVEL50);
						if ((lv50_step == L1Quest.QUEST_END) || (lv50_step >= 4)) { // 50级任务结束
							htmlid = "kimaq4";
						} else if (lv50_step == 3) {
							htmlid = "kimaq3";
						} else {
							if (lv50_step >= 1) { // 需先找布鲁迪卡，才能进行后续任务
								htmlid = "kimaq1";
			...
			} else if (npcid == 70824) { // アサシンマスターの追从者
				if (player.isDarkelf()) {
					if (player.getTempCharGfx() == 3634) { // アサシン变身
						int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
						if (lv45_step == 1) {
							htmlid = "assassin1";
						} else if (lv45_step == 2) {
				...
				if (player.isDarkelf()) { // ダークエルフ
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					if (lv45_step >= 5) { // ロジェ２回目同意济み
						htmlid = "roje14";
					} else if (lv45_step >= 4) { // イエティの头部 交换济み
						htmlid = "roje13";
					} else if (lv45_step >= 3) { // ロジェ 同意济み
						htmlid = "roje12";
					} else if (lv45_step >= 2) { // アサシンマスターの追从者 同意济み
						htmlid = "roje11";
					} else { // アサシンマスターの追从者 未同意
						htmlid = "roje15";
					}
				}
			} else if (npcid == 70811) { // ライラ
				if (quest.get_step(L1Quest.QUEST_LYRA) >= 1) { // 契约济み
					htmlid = "lyraEv3";
				} else { // 未契约
					htmlid = "lyraEv1";
				}
			 ...
			 // TODO 混沌精灵使者*/
                    ...
                    if(player.isElf() && player.getLevel() >= 80 && player.getElfAttr2() == 0 && player.getElfAttr() != 0){
                            htmlid = "jlssx1";
                    } else if(player.isElf() && player.getElfAttr() == 0){
                            htmlid = "jlssx2";
                    } else if(player.isElf() && player.getElfAttr2() != 0){
                            htmlid = "jlssx7";
                    } else if(player.isGm()){
                    		htmlid = "jlssx10";
                    } else {
			...
			} else if (npcid == 70998) { // 歌う岛のガイド
				htmlid = talkToSIGuide(player);
			} else if (npcid == 70999) { // アレックス(歌う岛)
				htmlid = talkToAlex(player);
			} else if (npcid == 71000) { // アレックス(训练场)
				htmlid = talkToAlexInTrainingRoom(player);
			} else if (npcid == 71002) { // キャンセレーション师
			...
			} else if (npcid == 71014) { // 村の自警团(右)
				if (player.getLevel() < 13) {
					htmlid = "en0241";
				}
			} else if (npcid == 71015) { // 村の自警团(上)
				if (player.getLevel() < 13) {
					htmlid = "en0261";
			...
			} else if (npcid == 71031) { // 佣兵ライアン
				if (player.getLevel() < 25) {
					htmlid = "en0081";
				}
			} else if (npcid == 71032) { // 冒险者エータ
				if (player.isElf()) {
					htmlid = "en0091e";
				...
				if (player.getInventory().checkItem(41227)) { // アレックスの绍介状
					if (player.isElf()) {
						htmlid = "en0201e";
					} else if (player.isDarkelf()) {
						htmlid = "en0201d";
				...
				if (player.getLevel() < 12) {
					htmlid = "jpe0114";
						...
						&& (player.getLevel() < 25)) {
					if (player.getInventory().checkItem(41239)) { // ヴィートへの手纸
						htmlid = "jpe0113";
					} else {
						htmlid = "jpe0111";
					}
				...
				if (player.getLevel() < 12) {
					htmlid = "jpe0133";
						...
						&& (player.getLevel() < 25)) {
					if (player.getInventory().checkItem(41240)) { // フェーダへの手纸
						htmlid = "jpe0132";
					} else {
						htmlid = "jpe0131";
					}
				...
				if (player.getLevel() < 10) {
					htmlid = "jpe0086";
						...
						&& (player.getLevel() < 25)) {
					if (player.getInventory().checkItem(41226)) { // パゴの药
						htmlid = "jpe0084";
					} else if (player.getInventory().checkItem(41225)) { // ケスキンの発注书
						htmlid = "jpe0083";
					} else if (player.getInventory().checkItem(40653)
							|| player.getInventory().checkItem(40613)) { // 赤い键・黑い键
						htmlid = "jpe0081";
					}
				}
				...
				if (player.getQuest().get_step(L1Quest.QUEST_DESIRE) == L1Quest.QUEST_END) {
					...
					htmlid = "minicod05";
				} else if ((player.getQuest().get_step(L1Quest.QUEST_SHADOWS) == L1Quest.QUEST_END // 影の神殿侧クエスト终了
				)
				...
				if (player.getQuest().get_step(L1Quest.QUEST_SHADOWS) == L1Quest.QUEST_END) {
					...
					htmlid = "minitos05";
				} else if ((player.getQuest().get_step(L1Quest.QUEST_DESIRE) == L1Quest.QUEST_END // 欲望の洞窟侧クエスト终了
				)
				...
				if (player.isDragonKnight()) { // ドラゴンナイト
					if ((player.getLevel() >= 50)
						...
						if (lv15_step == 0) {
							htmlid = "prokel2";
						} else if (lv15_step == L1Quest.QUEST_END) { // クリア济み)
							htmlid = "prokel7";
						} else {
							htmlid = "prokel4";
						}
				...
				/*
				 * } else if (npcid == 80145) { // 长老 シルレイン int lv45_step =
				 ...
				 * "silrein43"; } } }
				 */
			} else if (npcid == 81247) { // 第一 白蚂的尸体
				if (player.isIllusionist()) { // 幻术士 LV45 试练任务
					int lv30_step = quest.get_step(L1Quest.QUEST_LEVEL30);
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
			...
			} else if (npcid == 81310) { // 红色灵魂之火
				if (player.isDragonKnight()) { // 龙骑士 LV50 试炼任务
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					int lv50_step = quest.get_step(L1Quest.QUEST_LEVEL50);
			...
			} else if (npcid == 81311) { // 蓝色灵魂之火
				if (player.isIllusionist()) { // 幻术士 LV50 试炼任务
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					int lv50_step = quest.get_step(L1Quest.QUEST_LEVEL50);
					if (player.getLevel() > 49
							&& lv45_step == L1Quest.QUEST_END) {
			...
			} else if (npcid == 80145) { // 长老 希莲恩
				int lv15_step = quest.get_step(L1Quest.QUEST_LEVEL15);
				int lv30_step = quest.get_step(L1Quest.QUEST_LEVEL30);
				int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
				int lv50_step = quest.get_step(L1Quest.QUEST_LEVEL50);
				if (player.isDragonKnight()) { // 龙骑士
			...
			} else if (npcid == 81245) { //  妖魔密使(海音地监3楼)
				if (player.isDragonKnight()) {
					if (player.getTempCharGfx() == 6984) { // オーク密使变身
			...
			} else if (npcid == 81256) { // 修练场管理员
				int quest_step = quest.get_step(L1Quest.QUEST_TUTOR2);// 任务编号阶段
				int playerLv = player.getLevel();// 角色等级
				if (playerLv > 4 && playerLv < 13 && quest_step > 2) {
					newUserHelp(player, 2);// HASTE
				} else if (playerLv > 4 && playerLv < 13 && quest_step < 2) {
					if (playerLv < 12) {
						player.addExp(ExpTable.getNeedExpNextLevel(playerLv));// 给予
																				// up
																				// lv
																				// +1
					}
					newUserHelp(player, 2);// HASTE
					quest.set_step(L1Quest.QUEST_TUTOR2, 2); // 设定任务
					htmlid = "admin3";// 获得装备
					...
					if (playerLv < 6) {
						player.addExp(ExpTable.getNeedExpNextLevel(playerLv));// 给予
																				...
																				// lv
																				// +1
						newUserHelp(player, 3);// 神圣
					}
					quest.set_step(L1Quest.QUEST_TUTOR2, 1); // 设定任务
					htmlid = "admin2";
				} else if (playerLv < 5) {
					newUserHelp(player, 2);// HASTE
					newUserHelp(player, 3);// 神圣
			...
			} else if (npcid == 81260) { // 村庄福利员
				int townid = player.getHomeTownId();// 角色所属村庄
				if (player.getLevel() > 9 && townid > 0 && townid < 11) {
					htmlid = "artisan1";
				}
			} else if (npcid == 81261) { // 宝石加工师^大卫
				if (player.getInventory().checkItem(49031, 1)) { // 冰之结晶
	...
	private String talkToAlex(L1PcInstance pc) {
		...
		if (pc.getLevel() < 3) {
			...
			if (pc.getInventory().checkItem(41227)) { // アレックスの绍介状
				htmlid = "jpe0023";
			} else if (pc.isCrown()) {
				htmlid = "jpe0024p";
			} else if (pc.isKnight()) {
