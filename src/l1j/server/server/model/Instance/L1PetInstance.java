...
public class L1PetInstance extends L1NpcInstance {
	...
	public void liberate() {
		...
		if (_petMaster.getPetList().isEmpty()) {
			_petMaster.sendPackets(new S_PetCtrlMenu(false));// 关闭宠物控制图形介面
		}

		deleteMe();

		// DBとPetTableから削除し、ペットアミュも破弃
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		...
		perceivedFrom.sendPackets(new S_PetPack(this, perceivedFrom)); // ペット系オブジェクト认识
		if (isDead()) {
	...
	public void onAction(L1PcInstance pc, int skillId) {
		...
		if (master.isTeleport()) { // テレポート处理中
			return;
		}
		if (getZoneType() == 1) { // 攻击される侧がセーフティーゾーン
			L1Attack attack_mortion = new L1Attack(pc, this, skillId); // 攻击モーション送信
	...
	public void startFoodTimer(L1PetInstance pet) {
		...
		timer.scheduleAtFixedRate(_petFood, 1000, 20000); // 每 X秒减少
	}
