...
public class L1PcInventory extends L1Inventory {
				...
				if (ItemUpdate.getInstance().checkItem(item.getId()) != 0) {
					...
					for (L1WilliamItemUpdate item_update : itemUpdate.getItemUpdateList()) {
						if (item_update.getId() == item.getId()) {
							item.setUpdateCount(item_update.getCount()); // 可用卷軸次數
							item.setUpdateDmg(item_update.getAddDmg()); // 攻擊力
							item.setUpdateDmgModifier(item_update.getAddDmgModifier()); // 額外攻擊點數
							...
							item.setUpdateHp(item_update.getAddHp());//盔甲體力
							item.setUpdateMp(item_update.getAddMp());//盔甲魔力
	...
	//自行添加判断
	public boolean consumeEquippedCharge(int id) {
		for (Object itemObject : _items) {
			L1ItemInstance item = (L1ItemInstance) itemObject;
			if (item.getItem().getItemId() == id && item.isEquipped()) {
