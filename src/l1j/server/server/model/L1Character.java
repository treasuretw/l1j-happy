...
public class L1Character extends L1Object {
	...
	/**
	 * キャラクターの可视范围に居るプレイヤーへ、パケットを送信する。
	 * 
	 * @param packet
	...
	/**
	 * キャラクターの可视范围に居るプレイヤーへ、パケットを送信する。 ただしターゲットの画面内には送信しない。
	 * 
	 * @param packet
	 *            送信するパケットを表すServerBasePacketオブジェクト。
	 */
	...
	/**
	 * キャラクターの可视范围でインビジを见破れるor见破れないプレイヤーを区别して、パケットを送信する。
	 * 
	 * @param packet
	 *            送信するパケットを表すServerBasePacketオブジェクト。
	 * @param isFindInvis
	 ...
	 * @param ty
	 *            座标のY值
	 * @return 指定された座标に对する方向
	 */
	public int targetDirection(int tx, int ty) {
		float dis_x = Math.abs(getX() - tx); // Ｘ方向のターゲットまでの距离
		float dis_y = Math.abs(getY() - ty); // Ｙ方向のターゲットまでの距离
		float dis = Math.max(dis_x, dis_y); // ターゲットまでの距离
		if (dis == 0) {
	...
	/**
	 * キャラクターのスキル效果の持续时间を返す。
	 * 
	 * @param skillId
	 *            调べる效果のスキルID
	 * @return スキル效果の残り时间(秒)。スキルがかかっていないか效果时间が无限の场合、-1。
	 */
	 ...
	 * @param timer
	 *            ディレイ时间を表す、L1ItemDelay.ItemDelayTimerオブジェクト。
	 */
	 ...
	 * 
	 * @param delayId
	 *            调べるアイテムディレイID。 通常のアイテムであれば0、インビジビリティ クローク、バルログ ブラッディ
	 *            クロークであれば1。
	...
	/**
	 * キャラクターのアイテムディレイ时间を表す、L1ItemDelay.ItemDelayTimerを返す。
	 * 
	 * @param delayId
	 *            调べるアイテムディレイID。 通常のアイテムであれば0、インビジビリティ クローク、バルログ ブラッディ
	 *            クロークであれば1。
	 * @return アイテムディレイ时间を表す、L1ItemDelay.ItemDelayTimer。
	 */
	 ...
	 * キャラクターの从者リストを返す。
	 * 
	 * @return キャラクターの从者リストを表す、HashMapオブジェクト。このオブジェクトのKeyはオブジェクトID、
	...
	/**
	 * キャラクターが存在する座标が、どのゾーンに属しているかを返す。
	 * 
	 * @return 座标のゾーンを表す值。セーフティーゾーンであれば1、コンバットゾーンであれば-1、ノーマルゾーンであれば0。
	 */
	...
	/**
	 * 指定されたオブジェクトを、キャラクターが认识しているかを返す。
	 * 
	 * @param obj
	 *            调べるオブジェクト。
	 * @return オブジェクトをキャラクターが认识していればtrue、していなければfalse。 自分自身に对してはfalseを返す。
	 */
	...
	/**
	 * キャラクターが认识している全てのオブジェクトを返す。
	 * 
	 * @return キャラクターが认识しているオブジェクトを表すL1Objectが格纳されたArrayList。
	 */
	...
	/**
	 * キャラクターが认识している全てのプレイヤーを返す。
	 * 
	 * @return キャラクターが认识しているオブジェクトを表すL1PcInstanceが格纳されたArrayList。
	 */
	...
	public void setAc(int i) {
		_trueAc = i;
		_ac = IntRange.ensure(i, -211, 32767);// TODO 突破防御限制
	}
	...
	private int _dmgup = 0; // ● 近距离伤害补正（-128～127）
	...
	private int _bowDmgup = 0; // ● 弓伤害补正（-128～127）
	...
	public int getHitup() {
		return (_hitup
				+ L1MagicDoll.getHitAddByDoll(this)); // TODO 魔法娃娃效果 - 近距离的命中力增加
	}
	...
	private int _portalNumber = -1; // 龙之门扉编号
	...
	// 加入防加速系统 add
	/** 有向施法动作延迟 */
	private boolean _isSPELL_DIRDelay = false;

	public void setSPELL_DIRDelay(boolean flag) {
		_isSPELL_DIRDelay = flag;
	}

	public boolean isSPELL_DIRdelay() {
		return _isSPELL_DIRDelay;
	}
	/** 无向施法动作延迟 */
	private boolean _isSPELL_NODIRDelay = false;

	public void setSPELL_NODIRDelay(boolean flag) {
		_isSPELL_NODIRDelay = flag;
	}

	public boolean isSPELL_NODIRdelay() {
