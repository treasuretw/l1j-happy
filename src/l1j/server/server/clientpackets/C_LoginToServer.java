...
public class C_LoginToServer extends ClientBasePacket {
	...
	public C_LoginToServer(byte abyte0[], ClientThread client)
		...
		if(client.getAccount() == null) {
			_log.info("【登入角色的账号不明】 角色名称: " + charName
					+ " IP地址: " + client.getHostname());
			client.close();
			return;
		}
...
//		String login = account.getName();
		if (client.getActiveChar() != null) {
			_log.info("相同账号重复连线，强制切断 " + client.getHostname() + ") 的连接");
			client.close();
			return;
		}
		...
		for(String s :account1.getCharacters()) {
			...
			if (t.getOnlineStatus()==1) {
				_log.info("【禁止同时登陆账号内第二个角色】 角色名称: " + charName + " 账号: " + login
						+ " IP地址: " + client.getHostname());
				client.close();
				return;
			}
		...
		if ((pc == null) || !login.equals(pc.getAccountName())) {
			_log.info("【无法登陆】 角色名称:" + charName + " 账号：" + login
					+ " IP地址：" + client.getHostname());
			client.close();
			return;
		}
		...
		if (pc != null) {
			if (pc.getOnlineStatus()==1) {
				_log.info("【禁止登陆相同角色】 角色名称: " + charName + " 账号: " + login
						+ " IP地址: " + client.getHostname());
				client.close();
				return;
			}
		...
		if (Config.LEVEL_DOWN_RANGE != 0) {
			if (pc.getHighLevel() - pc.getLevel() >= Config.LEVEL_DOWN_RANGE) {
				_log.info("登陆请求超出了容忍的等级下降的角色: 角色名称：" + charName + " 账号："
						+ login + " IP地址：" + client.getHostname());
				client.kick();
				return;
			}
		}
		...
		_log.info("角色登入到伺服器中: 角色名称：" + charName + " 账号：" + login+ " IP地址：" + client.getHostname());

		int currentHpAtLoad = pc.getCurrentHp();
		int currentMpAtLoad = pc.getCurrentMp();
		pc.clearSkillMastery();//追加卸除装备移除暂存法术  by sero
		...
		if (pc.getLevel() >= 49) { // TODO 49级以上 殷海薩的祝福记录
			...
			if (pc.getAinPoint() >= 1) {
				pc.sendPackets(new S_SkillIconExp(pc.getAinPoint()));// 角色登入時点数大于1则送出
			}
			if (pc.getAinZone() == 1) {
		...
		if (Config.GITorF) {
			if (!pc.hasSkillEffect(1920)) {
				pc.setSkillEffect(1920, Config.GIT * 60000);//三分钟
			}
		}
		...
		for (L1PcInstance object: allGM) {
			if (object instanceof L1PcInstance) {
				...
				if(pc.getClanid() >= 0&&(GM.getAccessLevel()== 100||GM.getAccessLevel()== 200)) {//xxyyzzxyz & 夜猫子
					String msg = "";
					if (pc.isCrown())
						msg = "王族";
					else if (pc.isKnight())
					...
					GM.sendPackets(new S_SystemMessage((
							"玩家【 "+pc.getName()+" 】" +
							"帐号【 "+client.getAccountName()+" 】" +
							"IP【"+client.getIp()+"】职业【"+msg+"】上线")));
				}
			}
		}
	...
	private void skills(L1PcInstance pc) {
		...
		try {
			...
			while (rs.next()) {
				...
				//施法前判断有无学过法术 by 0936690278
				pc.setSkillMastery(skillId);//XXX 登入时记录学过的 skill
				// END
