...
public class ItemMagic {
	private static Logger _log = Logger.getLogger(ItemMagic.class.getName());

	private static ItemMagic _instance;

	private final HashMap<Integer, L1WilliamItemMagic> _itemIdIndex = new HashMap<Integer, L1WilliamItemMagic>();
	...
	private void loadItemMagic() {
		...
		try {
			...
			_log.log(Level.SEVERE, "error while creating william_item_magic table",e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	private void fillItemMagic(ResultSet rs) throws SQLException {
		while (rs.next()) {
			...
			int checkClass = rs.getInt("checkClass"); // 检查可用职业
			int checkItem = rs.getInt("checkItem"); // 携带物品判断
			int skill_id = rs.getInt("skill_id"); // 魔法 ID
