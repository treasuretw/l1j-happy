...
//import l1j.ABC.DragonGate.Teleport;
//import l1j.ABC.DragonGate.WaitTime;
//import l1j.ABC.DragonGate.isOver;
...
public class DragonGate extends TimerTask {

	private static Logger _log = Logger.getLogger(DragonGate.class.getName());
	...
	//***副本 部分***
	int gateNpcid = 99008;//副本 传送门编号
	L1Npc gatenpc = NpcTable.getInstance().getTemplate(gateNpcid);

	private static DragonGate _instance;

	public static DragonGate getStart() {
		if (_instance == null) {
			_instance = new DragonGate();
		}
		return _instance;
	}

	public void startDragonGate(L1PcInstance pc) {
	 ...
	 * 清空副本资讯(副本结束)
	 */
	public void DragonDead() {	//怪死亡后关闭
		sendMessage("\\fY任务已完成，即将传往下一关。");
		isGameOver();
	}
	...
	private void closeGate() {//关闭传送门
		for (L1Object obj : L1World.getInstance().getVisibleObjects(_doormap).values()) {
			if (obj instanceof L1NpcInstance) {
				...
				if (Cracknpc.getNpcId()== 99008 ) { //副本传送门
					Cracknpc.deleteMe();
				}
			}
		}
	...
	private void clearMap() {	//清除地图中的:玩家、NPC、道具
		...
		for(L1PcInstance pc : GatePcList) {
			...
			//	L1Teleport.teleport(pc,33442,32797,(short)4,5,true);//副本结束，传送的地图 预设奇岩
				L1Teleport.teleport(pc,32724,32830,(short)207,5,true);//副本结束，传送的地图 预设奇岩
			}
		}
		for (Object obj : L1World.getInstance().getVisibleObjects(206).values()) {
	...
	private void spawnGate() {			//创造传送门
		...
		//crack = new L1Location(32678, 32745, 1005); //点击门后传送到的坐标
		crack = new L1Location(32724, 32830, 206); //点击门后传送到的坐标
	...
	/*回传目前状态*/
	public static int getStatus = 0;
	public static int getStatus() {
		getStatus = _DragonGateStatus;
		return getStatus;
	}
	/*回传目前状态*/
